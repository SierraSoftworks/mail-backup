...
pub struct StreamOptions {
    ...
    /// How often to run a sync even without any notifications (belt and
    /// braces against missed events). The configured cron schedule, when
    /// present, also triggers syncs.
...
pub(crate) async fn stream_phase<S: MailSource, T: MailStore>(
    ...
    let mut next_safety_poll = next_poll_instant(schedule, stream_options);
    ...
    loop {
        ...
        if due || now >= next_safety_poll {
            if now >= next_safety_poll {
                debug!(
                    "Running the scheduled safety synchronization for {}",
                    policy
                );
...
fn next_poll_instant(schedule: Option<&croner::Cron>, options: &StreamOptions) -> Instant {
    let from_schedule = schedule
    ...
    Instant::now()
        + from_schedule
            .unwrap_or(options.safety_poll)
            .min(options.safety_poll)
}
